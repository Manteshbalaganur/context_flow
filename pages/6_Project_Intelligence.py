...
from services.databricks_service import pipeline_connection_status, query_pipeline_insights, test_pipeline_connection
style(); p,_,a=selected_data(); st.title("Project intelligence")
connection=pipeline_connection_status()
insights,error=query_pipeline_insights(p.project_id) if connection["connected"] else ([], connection["message"])
if not a:
    st.info("No local CheckIN analysis yet. Databricks insights are shown below when available.")
else:
...
if connection["connected"]:
    if st.button("Test Databricks connection"):
        ok,message=test_pipeline_connection()
